/*
 * Sonarr
 *
 * Sonarr API docs - The v3 API docs apply to both v3 and v4 versions of Sonarr. Some functionality may only be available in v4 of the Sonarr application.
 *
 * The version of the OpenAPI document: 3.0.0
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = Sonarr.OpenAPI.Client.OpenAPIDateConverter;

namespace Sonarr.OpenAPI.Model
{
    /// <summary>
    /// EpisodesMonitoredResource
    /// </summary>
    [DataContract(Name = "EpisodesMonitoredResource")]
    public partial class EpisodesMonitoredResource : IEquatable<EpisodesMonitoredResource>, IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="EpisodesMonitoredResource" /> class.
        /// </summary>
        /// <param name="episodeIds">episodeIds.</param>
        /// <param name="monitored">monitored.</param>
        public EpisodesMonitoredResource(List<int> episodeIds = default(List<int>), bool monitored = default(bool))
        {
            this.EpisodeIds = episodeIds;
            this.Monitored = monitored;
        }

        /// <summary>
        /// Gets or Sets EpisodeIds
        /// </summary>
        [DataMember(Name = "episodeIds", EmitDefaultValue = true)]
        public List<int> EpisodeIds { get; set; }

        /// <summary>
        /// Gets or Sets Monitored
        /// </summary>
        [DataMember(Name = "monitored", EmitDefaultValue = true)]
        public bool Monitored { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class EpisodesMonitoredResource {\n");
            sb.Append("  EpisodeIds: ").Append(EpisodeIds).Append("\n");
            sb.Append("  Monitored: ").Append(Monitored).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="input">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object input)
        {
            return this.Equals(input as EpisodesMonitoredResource);
        }

        /// <summary>
        /// Returns true if EpisodesMonitoredResource instances are equal
        /// </summary>
        /// <param name="input">Instance of EpisodesMonitoredResource to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(EpisodesMonitoredResource input)
        {
            if (input == null)
                return false;

            return 
                (
                    this.EpisodeIds == input.EpisodeIds ||
                    this.EpisodeIds != null &&
                    input.EpisodeIds != null &&
                    this.EpisodeIds.SequenceEqual(input.EpisodeIds)
                ) && 
                (
                    this.Monitored == input.Monitored ||
                    this.Monitored.Equals(input.Monitored)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                int hashCode = 41;
                if (this.EpisodeIds != null)
                    hashCode = hashCode * 59 + this.EpisodeIds.GetHashCode();
                hashCode = hashCode * 59 + this.Monitored.GetHashCode();
                return hashCode;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        public IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
